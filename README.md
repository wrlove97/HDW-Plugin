...
### 20250901 -v7.2 🆕
- 原地挂机逻辑重构：智能返回策略，超出范围或无目标时立即返回挂机点
- 医生模式优化：智能技能释放，只释放目标缺少的状态技能，彻底解决重复释放问题

### 20250831 -v7.1
- 完善智能跟随系统逻辑：双层跟随策略（强制+逻辑），基于搜索范围的智能距离检测
- 跟随范围限制最大25格，超过5格强制跟随，5格内正常游戏逻辑
- 优先跟随其次打怪，确保有效距离内不会乱跑

### 20250831 -v7.0
**重大功能升级 - 物品拾取系统重构**
- 指定物品拾取模式：支持精确控制拾取目标，避免拾取无用物品
- 拖拽式物品管理：支持将游戏内道具直接拖拽到输入框
- 对接API实现按物品名称精确拾取，UI界面重构优化
